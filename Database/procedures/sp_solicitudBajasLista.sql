DELIMITER //
CREATE definer = prosigo@`%` procedure sp_solicitudBajasLista()
BEGIN
    SELECT
        *,
    ...
    FROM (
        SELECT
            ...
            IFNULL(
                (
                    ...
                    INNER JOIN bitacora.grupo_cobranza gc ON gcc.grupo_cobranza_id = gc.id
                    WHERE gcc.credito_id = Prestamo.PQ_CLAVE
                    LIMIT 1
                ),
                ''
            ) AS marca_grupo_cobranza
        FROM Socios
        ...
        WHERE
            ...
            /* ============================================================
               AQUÍ FILTRAMOS ÚNICAMENTE POR LOS CRÉDITOS ASIGNADOS A ESE GRUPO
               ============================================================ */
            AND EXISTS (
                SELECT 1
                FROM bitacora.solicitud_baja gcc
                WHERE gcc.CreditoId = Prestamo.PQ_CLAVE
            )
